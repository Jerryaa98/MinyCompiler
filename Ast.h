...
static int IsInsideOperation = OFF;
//====================================================================
//Record Features
...
static std::vector<OperationParameter> reverseAddressValues(std::vector<OperationParameter> v) {
    // create an empty vector to store the reversed elements
    //std::vector<OperationParameter> reversed = v;
    std::vector<int> AddressVector;

    for(auto var : v)
    {
        AddressVector.push_back(var.Address);
    }

    for(auto i : v)
    {
        i.Address = AddressVector.back();
        AddressVector.pop_back();
    }
   /* // iterate through the original vector in reverse order and add each element to the reversed vector
    for (int i = v.size() - 1; i >= 0; i--) {
        int temp = reversed.at(i).Address;
        reversed.at(v.size() - 1 - i).Address = temp;
    }*/
...
static int OperationParameterCurrentAddress = 5;

static int ByReferenceDetector = OFF;
static int ByValueDetector = OFF;
...
public:
    ...
    void pcodegen(ostream& os) {
        ...
        if(Condition == Declare)
        {
            ...
            {
                {
                    ...
                    input.Name = *name_;
                    input.Address = OperationParameterCurrentAddress;
                    input.Type = Simple;
                    input.Size = 1;
            ...
            if(DetectorOfVariable != Record && DetectorOfVariable != Array)
            {
                OperationParameterCurrentAddress++;
            }
        }
        /*if(Condition == State)
        {
...
public:
    ...
    void pcodegen(ostream& os) {
        ...
        /*Swap Addresses Values in reverse order
        reverseAddressValues(OperationParametersList);*/
        os << *name_ << "_begin :" << endl;
        block_->pcodegen(os);
        OperationIndex--;
        IsInsideOperation = OFF;
        os << "retf" << endl;
...
public:
    ...
    void pcodegen(ostream& os) {
        ...
        IsInsideOperation = ON;
        assert(block_);
        int temp = OperationParameterCurrentAddress;
        if (formal_list_) {
            OperationParameterCurrentAddress = 5;
            formal_list_->pcodegen(os);
        }
        OperationParameterCurrentAddress = temp;
